...
try:
    from wggan import Generator 
    from utils import check_create_dir 
except ImportError as e:
    print(f"Error importing modules. Make sure wggan.py and utils.py are in the src directory: {e}")
    sys.exit(1)
...
def generate_images(generator_path, output_dir, num_images, latent_dim, feature_maps_g, batch_size, device):
    ...
    with torch.no_grad():
        while generated_count < num_images:
            ...
            noise = torch.randn(current_batch_size, latent_dim, device=device).unsqueeze(2).unsqueeze(3)
