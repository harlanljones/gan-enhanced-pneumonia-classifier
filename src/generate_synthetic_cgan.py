...
try:
    ...
    print(f"Error importing modules. Make sure cgan.py and utils.py are in the src directory: {e}")
    sys.exit(1)

def generate_images(generator_path, output_dir, num_images, latent_dim, feature_maps_g, batch_size, device):
    check_create_dir(output_dir)
    num_classes = 2
    netG = Generator(latent_dim, num_classes, 3, feature_maps_g).to(device) 
    try:
    ...
    with torch.no_grad():
        while generated_count < num_images:
            ...
            labels = torch.randint(0, num_classes, (current_batch_size,), dtype=torch.long, device=device)

            fake_images = netG(noise, labels, alpha=1.0)
